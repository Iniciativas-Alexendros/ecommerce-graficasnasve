...
// 6 placeholders cuando no hay datos en Supabase
const PLACEHOLDERS: PortfolioItem[] = Array.from({ length: 6 }, (_, i) => ({
  id: `placeholder-${i}`,
  created_at: '',
  titulo: `Proyecto ${i + 1}`,
  descripcion: 'Próximamente',
  categoria: 'offset' as CategoriaPortfolio,
  imagen_url: '',
  imagen_alt: null,
  cliente: null,
  destacado: false,
...
export function PortfolioGrid({ items }: PropiedadesPortfolioGrid) {
  ...
  const itemsFiltrados =
    filtroActivo === 'todos'
      ? datos
      : datos.filter((item) => item.categoria === filtroActivo)
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 stagger">
          {itemsFiltrados.map((item) => (
            <button
              ...
              className="group text-left focus-visible:outline focus-visible:outline-2 focus-visible:outline-oro focus-visible:outline-offset-2"
              aria-label={`Ver ${item.titulo}`}
              disabled={esPlaceholder}
            >
              {/* Imagen o placeholder */}
              <div className="relative w-full aspect-[4/3] bg-fondo-alt overflow-hidden">
                {item.imagen_url ? (
                  <Image
                    src={item.imagen_url}
              ...
              <div className="pt-4 pb-2">
                ...
                {item.cliente && (
                  <p className="font-sans text-xs text-gris mt-1">{item.cliente}</p>
                )}
              </div>
            </button>
          ))}
      ...
      <dialog
        ref={dialogRef}
        className="fixed inset-0 m-auto max-w-4xl w-full bg-negro p-0 border-0 backdrop:bg-negro/80"
        onClick={(e) => {
        ...
        {itemSeleccionado && (
          <div className="relative">
            <button
              ...
              className="absolute top-4 right-4 z-10 w-10 h-10 flex items-center justify-center bg-negro/60 text-papel hover:bg-negro transition-colors"
            ...
            {itemSeleccionado.imagen_url ? (
              ...
              <div className="w-full aspect-video bg-fondo-alt flex items-center justify-center">
                <span className="font-mono text-xs text-gris">[imagen]</span>
              </div>
            )}
            ...
            <div className="p-6">
              ...
              <h2 className="font-display text-2xl font-bold text-papel">
              ...
              {itemSeleccionado.cliente && (
                <p className="font-sans text-xs text-gris mt-1">
                  Cliente: {itemSeleccionado.cliente}
                </p>
              )}
            </div>
          </div>
